here::i_am("Code/co-clustering.R")
library(here)
...
# Read group data
sj_1 <- read.csv(here("mNDP Results", "Sj_sim_cell_res1.txt"), header = F)
sj_2 <- read.csv(here("mNDP Results", "Sj_sim_cell_res2.txt"), header = F)
sj_3 <- read.csv(here("mNDP Results", "Sj_sim_cell_res3.txt"), header = F)
...
write.csv(distributionalCCM, file = here("For Collaborator", "Cell Groups - Posterior Co-clustering probability matrix.csv"))

h <- hclust(dist(distributionalCCM))
plot(h)
...
dist_hm_df$col <- rep(cutree(h, 6), length(groups))
...
(p1 <- ggplot(as.data.frame(dist_hm_df), aes(x = factor(obs1, levels =  dist_idx, ordered = T),
  ...
  labs(fill = "Co-clustering\n  probability", title = "Cell - System Level"))

# Plot distributional level groups
(p1a <- ggplot(as.data.frame(dist_hm_df),
              aes(x = factor(obs1, levels =  dist_idx, ordered = T),
                  y = factor(obs2, levels =  dist_idx, ordered = F),
                  fill = ifelse(cc_prob > quantile(cc_prob, 0.5), cc_prob, 0))) +
    geom_tile() + theme_bw() +
    theme(axis.ticks = element_blank(), axis.title = element_blank(),
          axis.text.x = element_blank(), panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
    ...
    scale_fill_gradient(low = "#ffffff", high = "#000000") +
    labs(fill = "Co-clustering\n  probability",
         subtitle = "Cutoff p > q_0.5"))

(p1b <- ggplot(as.data.frame(dist_hm_df),
               aes(x = factor(obs1, levels =  dist_idx, ordered = T),
                   y = factor(obs2, levels =  dist_idx, ordered = F),
                   fill = ifelse(cc_prob > quantile(cc_prob, 0.75), cc_prob, 0))) +
    geom_tile() + theme_bw() +
    theme(axis.ticks = element_blank(), axis.title = element_blank(),
          axis.text.x = element_blank(), panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
    ...
    scale_fill_gradient(low = "#ffffff", high = "#000000") +
    labs(fill = "Co-clustering\n  probability",
         subtitle = "Cutoff p > q_0.75"))

cell_dist_clust_df <- data.frame(RNA_number_id = rownames(df))
cell_dist_clust_df$chain1 <- res1$Sj
cell_dist_clust_df$chain2 <- res2$Sj
cell_dist_clust_df$chain3 <- res3$Sj
cell_dist_clust_df$posterior_clusters <- cutree(h, k = 6)

write.csv(cell_obs_clust_df, here("For Collaborator" ,"cell distribution clusters.csv"))

# Observational level
sij1 <- read.csv(here('mNDP Results', "sj_obs_sim_cell_res1.txt"), header = F)
sij2 <- read.csv(here('mNDP Results', "sj_obs_sim_cell_res2.txt"), header = F)
sij3 <- read.csv(here('mNDP Results', "sj_obs_sim_cell_res3.txt"), header = F)
sij_c <- as.numeric(as.matrix(rbind(sij1, sij2, sij3))); rm(sij1, sij2, sij3)

rij1 <- read.csv(here('mNDP Results', "Rij_sim_cell_res1.txt"), header = F)
rij2 <- read.csv(here('mNDP Results', "Rij_sim_cell_res2.txt"), header = F)
rij3 <- read.csv(here('mNDP Results', "Rij_sim_cell_res3.txt"), header = F)
...
write.csv(observationalCCM, file = here("For collaborator", "Cell Observations - Posterior Co-clustering probability matrix.csv"))

h <- hclust(dist(observationalCCM))
plot(h)
abline(h = 0.5, col = "red")
obs_idx <- rownames(df)[order.hclust(h)]
obs_hm_df <- expand_grid(RNA_number_id = rownames(df), RNA_number_id2 = rownames(df))
obs_hm_df$col <- rep(cutree(h, 5), length(rownames(df)))
...
(p2 <- ggplot(as.data.frame(obs_hm_df), aes(x = factor(RNA_number_id, levels =  obs_idx, ordered = T),
  ...
  scale_fill_gradient(low = "#ffffff", high = "#000000") +
  labs(fill = "Co-clustering\n  probability", title = "Cell - Observational Level",
       subtitle = "No cut off"))

# Plot of observational posterior co-clustering probabilities
(p2a <- ggplot(as.data.frame(obs_hm_df), aes(x = factor(RNA_number_id, levels =  obs_idx, ordered = T),
                                            y = factor(RNA_number_id2, levels =  obs_idx, ordered = F),
                                            fill = ifelse(cc_prob > quantile(cc_prob, 0.5), cc_prob, 0))) +
    geom_tile() + theme_bw() +
    theme(axis.ticks = element_blank(), axis.title = element_blank(),
    ...
    scale_fill_gradient(low = "#ffffff", high = "#000000") +
    labs(fill = "Co-clustering\n  probability",
         subtitle = "Cut off p > q_0.5"))

# Plot of observational posterior co-clustering probabilities
(p2b <- ggplot(as.data.frame(obs_hm_df), aes(x = factor(RNA_number_id, levels =  obs_idx, ordered = T),
                                             y = factor(RNA_number_id2, levels =  obs_idx, ordered = F),
                                             fill = ifelse(cc_prob > quantile(cc_prob, 0.75), cc_prob, 0))) +
    geom_tile() + theme_bw() +
    theme(axis.ticks = element_blank(), axis.title = element_blank(),
    ...
    scale_fill_gradient(low = "#ffffff", high = "#000000") +
    labs(fill = "Co-clustering\n  probability",
         subtitle = "Cut off p > q_0.75"))

p1 + p2
plots <- p1 + p1a + p1b +  p2 +p2a + p2b
ggsave(here("For Collaborator", "Cell Heatmaps.pdf"), plots, width = 30, height = 20)

# Data frame with the different clustering results - observational
cell_obs_clust_df <- data.frame(RNA_number_id = rownames(df))
cell_obs_clust_df$chain1 <- paste(res1$sj_obs, res1$rij, sep = "_")
cell_obs_clust_df$chain2 <- paste(res2$sj_obs, res2$rij, sep = "_")
cell_obs_clust_df$chain3 <- paste(res3$sj_obs, res3$rij, sep = "_")
cell_obs_clust_df$posterior_clusters <- cutree(h, k = 5)
write.csv(cell_obs_clust_df, here("For Collaborator" ,"cell observation clusters.csv"))
...
dist_hm_df$col <- rep(cutree(h, 3), 9)

# Plot distributional level groups
p1 <- ggplot(as.data.frame(dist_hm_df), aes(x = factor(obs1, levels =  dist_idx, ordered = T),
                                            y = factor(obs2, levels =  dist_idx, ordered = F), fill = cc_prob)) +
  geom_tile(size = 0.5) + theme_bw() +
  theme(axis.ticks = element_blank(), axis.title = element_blank(),
        axis.text.x = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        ...
        panel.background = element_blank()) +
  scale_fill_gradient(low = "#ffffff", high = "#000000") +
  labs(fill = "Co-clustering\n  probability", title = "System Level",
       subtitle = "No cutoff")
...
tissue_dist_clust_df$posterior_clusters <- cutree(h, k = 3)
tissue_dist_clust_df$chain1 <- res1$Sj
tissue_dist_clust_df$chain2 <- res2$Sj
tissue_dist_clust_df$chain3 <- res3$Sj
...
colnames(observationalCCM) <- rownames(observationalCCM) <- rownames(df)
write.csv(observationalCCM, file = here("For collaborator", "Tissue Observations - Posterior Co-clustering probability matrix.csv"))
...
obs_hm_df <- expand_grid(RNA_number_id = rownames(df), RNA_number_id2 = rownames(df))
obs_hm_df$cc_prob <- as.numeric(observationalCCM)
obs_hm_df$col <- rep(cutree(h, k= 3), length(rownames(df)))
...
(p2 <- ggplot(as.data.frame(obs_hm_df), aes(x = factor(RNA_number_id, levels =  obs_idx, ordered = T),
    ...
    theme(axis.ticks = element_blank(), axis.title = element_blank(),
          ...
          panel.background = element_blank(), axis.text.y = element_blank()) +
    scale_fill_gradient(low = "#ffffff", high = "#000000") +
    labs(fill = "Co-clustering\n  probability", title = "Tissue - Observational Level",
         subtitle = "No cut off"))
...
plots <- p1 + p1a + p1b +  p2 +p2a + p2b
ggsave(here("For Collaborator", "Tissue Heatmaps.pdf"), plots, width = 30, height = 20)

# Data frame with the different clustering results - observational
tissue_obs_clust_df <- data.frame(RNA_number_id = rownames(df))
tissue_obs_clust_df$chain1 <- paste(res1$sj_obs, res1$rij, sep = "_")
tissue_obs_clust_df$chain2 <- paste(res2$sj_obs, res2$rij, sep = "_")
tissue_obs_clust_df$chain3 <- paste(res3$sj_obs, res3$rij, sep = "_")
tissue_obs_clust_df$posterior_clusters <- cutree(h, k = 3)
write.csv(cell_obs_clust_df, here("For Collaborator" ,"Tissue clusters.csv"))
